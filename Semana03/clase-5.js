...
function mostrarDatosEnPerfil(albumes) {
    ...
    let cantidadAlbum = albumes.length
    let fav = albumes.filter(album => album.like).length
    
    let spanAlbum = document.getElementById("cant-albums")
    let spanFav = document.getElementById("cant-favoritos")

    spanAlbum.textContent = `${cantidadAlbum} álbum${cantidadAlbum !== 1 ? 'es' : ''}`
    spanFav.textContent = `${fav} favorito${fav !== 1 ? 's' : ''}`
