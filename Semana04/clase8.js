...
function renderizarErrores(listado) {
  ...
  if (cajaDeErrores) {
    // si cajaDeErrorres me da null... no pasa nada.. pero si existiere con esto evito👇🏼
  ...
  if (listado.length > 0) {
    ...
    divTemplate.style =
      "background:rgba(255, 0, 0, 0.2);padding: 0.5em 1em; color: red; margin: 0.5em 0";
...
function mostrarMensajeExito(listado) {
    ...
    if (listado.length == 0) {
        // const divCheck = document.createElement("div");
        divCheck.setAttribute("id", "exito");
        divCheck.style =
        "background:rgba(0, 255, 0, 0.2);padding: 0.5em 1em; color: green; margin: 0.5em 0";

        divCheck.innerHTML = "<p>¡Formulario completado con éxito! ✅</p>";
        form.appendChild(divCheck);

        const submitBotton = form.querySelector("[type='submit']");
        submitBotton.disabled = true;

        const box = document.getElementById("exito");
